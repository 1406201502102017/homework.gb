...
class Tree {
    ...
    public boolean delete(int id) {
        ...
        if (current.leftChild == null && current.rightChild == null) { // Если узел не имеет потомков
            if (current == null) {
                root = null;
            } else if(isLeftChild) {
        ...
        else if(current.rightChild == null) { // Если нет правого потомка
            if (current == null) {
        ...
        else if (current.leftChild == null) { // Если нет левого потомка
            if (current == null) {
    ...
    public void traverse(int traverseType) {
        switch(traverseType) {
            case 1:System.out.println("Обход предзаказа");
                preOrder(root);
                break;
        }
    }
...
public class BinaryTree {
    ...
    public static void main(String[] args) throws IOException {
        ...
        while(true) {
            System.out.print("Ввести первую букву показать, вставить, найти, удалить или пройти: ");
            int choice = getChar();
            switch(choice) {
                case 's':
                    theTree.displayTree();
                    break;
                case 'i':
                    System.out.print("Ввести значение для вставки: ");
                    value = getInt();
                    theTree.insert(new Person());
                    break;
                case 'f':
                    System.out.print("Ввести значение, чтобы найти: ");
                    value = getInt();
